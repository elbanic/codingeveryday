...
func getAncestors(n int, edges [][]int) [][]int {
	...
	for j := 0; j < len(edges); j++ {
		m[edges[j][1]] = append(m[edges[j][1]], edges[j][0])
	}

	var ret [][]int
	for i := 0; i < n; i++ {
		ret = append(ret, removeDupAndSortInt(getParents(m, i, []int{})))
	}
	return ret
}

func removeDupAndSortInt(intSlice []int) []int {
...
func getParents(m map[int][]int, key int, output []int) []int {
	parents, ok := m[key]
	if !ok {
		return output
	}

	output = append(output, parents...)
	for i := 0; i < len(parents); i++ {
		output = getParents(m, parents[i], output)
	}
